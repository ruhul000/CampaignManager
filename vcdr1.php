...
if ($mistype == 1){
	...
	if ($cntsid != 000){
		$sqlquery = "select voting_name from voting_detail where voting_id='" . $cntsid . "'";
		$result = mysql_query($sqlquery) or die('mysql error:' . mysql_error());
					...
					<td>
						<?if ($mistype == 1){?>
							<table align="left" border="0" cellspacing="1" cellpadding="0" width="748px" bgcolor="#525200" style="table-layout:fixed;">
								...
								<?for ($i=1;$i<=$day;$i++){
									...
									$sqlquery = "select * from file_detail where day(cdrdate)='" . $ii . "' and month(cdrdate)='" . $month . "' and year(cdrdate)='" . $year . "' and foldername='" . $cntsname . "' and type='2'";
									echo $sqlquery;
									//die();
									$result = mysql_query($sqlquery) or die('mysql error:' . mysql_error());
									...
									<tr bgcolor="#D9D9A8">
										...
										<td align="center" class="WorkBlueNormal">
											<?if ($fileexist == 0){?>
												No CDR Exist
											<?}else{?>
												<a class="WorkBlueNormal" href='contestcdr/<? echo $cntsname?>/<? echo $cdr_file?>.zip'>Download</a>
											<?}?>
										</td>
									</tr>
								<?}?>
						...
						<?}else{?>
							<table align="left" border="0" cellspacing="1" cellpadding="0" width="748px" bgcolor="#525200" style="table-layout:fixed;">
								...
								<tr bgcolor="#D9D9A8">
									<td background="images/welcomestrip.gif" align="center" align="center" class="WorkBlk"><b>Date</b></td>
									<td background="images/welcomestrip.gif" align="center" align="center" class="WorkBlk"><b>Voting Name</b></td>
									<td background="images/welcomestrip.gif" align="center" align="center" class="WorkBlk"><b>Download</b></td>
								</tr>
								...
								<?for ($i=1;$i<=$day;$i++){
									...
									$daytoshow = $ii . "/" . $month . "/" . $year;
									$cdr_file = "VotingCDR_" . $month . $ii . $year;
									...
									<tr bgcolor="#D9D9A8">
										...
										<td align="center" class="WorkBlueNormal">
											<?if ($fileexist == 0){?>
												No CDR Exist
											<?}else{?>
												<a class="WorkBlueNormal" href='votingcdr/<? echo strtolower($cntsname)?>/<? echo $cdr_file?>.zip'>Download</a>
											<?}?>
										</td>
									</tr>
								<?}?>
