...
if($action==3 && $cntsid){
	...
	header("Location: voting_question_view.php?login=" . $login_form . "&sess_id=" . $sess_id ."&msg_alert=" . $msg_alert ."&action=1&treeview_cod=" . $treeview_cod ."&cnts_id=" . $cntsid . "&question_id=" . $qutn_id . "&smenu=" . $smenu);
	die();
}
...
if($action==1){
	...
	header("Location: voting_question_view.php?login=" . $login_form . "&sess_id=" . $sess_id . "&msg_alert=" . $msg_alert ."&cnts_id=" . $cntsid ."&question_id=" . $q_id . "&smenu=" . $smenu);
	die();
}else if($action==2){
	update_voting_question ();
	$msg_alert = "Voting Question successfully Modified!!!";
	header("Location: voting_question_modify.php?login=" . $login_form . "&sess_id=" . $sess_id . "&msg_alert=" . $msg_alert ."&cnts_id=" . $cntsid ."&question_id=" . $qutn_id ."&flag=1&smenu=" . $smenu);
	die();
}
...
function insert_voting_question (){
	...
	$sqlquery = "insert into voting_questions(voting_id, question, a, b, c, d, active_status,ques_no,max_options,voting_seq,login) values('" . $cntsid . "', '" . str_replace("'", "''", $cnts_question) . "', '" . str_replace("'", "''", $opt_a) . "', '" . str_replace("'", "''", $opt_b) . "', '" . str_replace("'", "''", $opt_c) . "', '" . str_replace("'", "''", $opt_d) . "', '1', '" . $qno . "', '" . $max_option . "', '" . $selected_seq . "','" . $login_form . "')";
	//echo $sqlquery;
	...
	if ($result == 0){
		...
		header("Location: voting_question_view.php?login=" . $login_form . "&sess_id=" . $sess_id ."&msg_alert=" . $msg_alert ."&action=" . $action ."&treeview_cod=" . $treeview_cod ."&header1=" . $hdr1 . "&cnt=1&smenu=" . $smenu);
		die();
	}else{
...
function update_voting_question(){
	...
	$sqlquery = "update voting_questions set question='" . str_replace("'", "''", $cnts_question) . "', a='" . str_replace("'", "''", $opt_a) . "', b='" . str_replace("'", "''", $opt_b) . "', c='" . str_replace("'", "''", $opt_c) . "', d='" . str_replace("'", "''", $opt_d) . "', active_status='" . $act_status . "' where id='" . $qutn_id . "' and voting_id='" . $cntsid . "' and login='" . $login_form . "'";
	$result = mysql_query($sqlquery) or die('mysql error:' . mysql_error());

	if ($result == 0){
		$msg_alert = "There is some problem at server";
		user_session($login_form,$sess_id,$msg_alert);
		header("Location: voting_question_modify.php?login=" . $login_form . "&sess_id=" . $sess_id . "&msg_alert=" . $msg_alert ."&action=2&treeview_cod=" . $treeview_cod . "&smenu=" . $smenu);
		die();
	}else{
...
function delete_voting_question(){
	...
	$sqlquery = "delete from voting_questions where id='" . $qutn_id . "' and voting_id='" . $cntsid . "' and login='" . $login_form . "'";
