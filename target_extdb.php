...
if (!$trgt_name && $retval){
...
}else if(is_file("connection".DIR_SEPERATOR."$login_form".DIR_SEPERATOR."connection.php") && $retval){
	require_once("connection".DIR_SEPERATOR."$login_form".DIR_SEPERATOR."connection.php");
	...
	copy ("connection".DIR_SEPERATOR."$login_form".DIR_SEPERATOR."connection.php","$temp_path".DIR_SEPERATOR."connection.php");

	$path=$temp_path . DIR_SEPERATOR . $yr . $mon . $da . $hor . $min . $ss . '.csv';
...
//		echo $sqlquery;
		...
		$temp=MYSQL." -N -h" . $location . " -u" . $username . " -p" . $password . " " . $database . " -e \"" . $sqlquery . ";\">" . $path;
		$output = exec($temp);
...
function check_target_availability($target_name)
{
	...
	$result = mysql_query($sqlquery) or die('mysql error:2' . mysql_error());
	$trg_avail = 0;
...
function insert_target_detail($trgt_name,$path,$grp_id,$sgrp_id,$daily_target,$sqlquery)
{
	...
	//echo $sqlquery;
	$sqlquery = "insert into target_detail (target_name,file_path,group_id,subgroup_id,target_status,login,daily_new_target,target_query,date_field,cron_start_date,target_date) values ('" . addslashes($trgt_name) . "','" . addslashes($path) . "','" . $grp_id . "','" . $sgrp_id . "','1','" . addslashes($login_form) . "','" . addslashes($daily_target) . "','" . $sqlquery . "','". $date_field . "',".$cron_start_date.",now())";
	$result = mysql_query($sqlquery) or die('mysql error:1' . mysql_error());
