...
echo "Company : ".$compName = $_REQUEST["companyName"];



$url = "login=" . $login_form ."&smenu=" . $smenu ."&sess_id=" . $sess_id . "&grp_name=" . $grp_name ."&grp_desc=" . $grp_desc . "&grp_id=" . $grp_id . "&sgrp_name=" . $sgrp_name ."&sgrp_desc=" . $sgrp_desc . "&sgrp_id=" . $sgrp_id. "&cpName=" . $compName;
if ($action == 1){
    $active_status=1;
    ...
    if ($group_exist == 0){
		...
		$strurl = "Location: subgroup_view.php?login=" . $login_form ."&smenu=" . $smenu ."&sess_id=" . $sess_id . "&sgrp_id=" . $sgrp_id . "&grp_id=" . $grp_id. "&cpName=" . $compName;
		header($strurl . "&msg_alert=" . $msg_alert);
		die();
    }
    ...
    $strurl = "Location: subgroup_view.php?login=" . $login_form ."&smenu=" . $smenu ."&sess_id=" . $sess_id . "&grp_id=" . $grp_id . "&sgrp_id=" . $sgrp_id. "&cpName=" . $compName;
...
if ($action == 1){
    ...
    header("Location: subgroup_view.php?login=" . $login_form ."&smenu=" . $smenu ."&sess_id=" . $sess_id . "&grp_id=" . $grp_id . "&sgrp_id=" . $sgrp_id . "&msg_alert=" . $msg_alert. "&cpName=" . $compName);
    die();
}elseif ($action == 2){
...
{
    global $login_form, $sess_id, $sgrp_name, $sgrp_desc, $grp_id,$active_status,$compName;

    $sgrp_desc = str_replace("'","''",$sgrp_desc);

    $sgrp_name = strtolower($sgrp_name);
    $sqlquery = "insert into subgroup_detail(login, group_id, subgroup_name, description, active_status,companyName) values('" . $login_form . "', '" . $grp_id. "', '" . $sgrp_name . "', '" . $sgrp_desc . "', '" . $active_status . "', '" . $compName. "')";
...
{
    global $login_form, $sess_id, $sgrp_desc, $grp_id, $sgrp_id, $active_status,$compName;

    $sgrp_desc = str_replace("'","''",$sgrp_desc);

    $sgrp_name = strtolower($sgrp_name);
    $sqlquery = "update subgroup_detail set description='" . $sgrp_desc . "', active_status='" . $active_status . "' where subgroup_id='" . $sgrp_id . "' and companyName='" . $compName. "'";
    $result = mysql_query($sqlquery) or die('mysql error:' . mysql_error());
    return 1;
}
...
{
    global $login_form,$grp_id,$sgrp_id,$sess_id,$compName;

    $sqlquery = "delete from subgroup_detail where subgroup_id='" . $sgrp_id . "' and companyName='" . $compName. "'";
    $result = mysql_query($sqlquery) or die('mysql error:' . mysql_error());
    return $grp_id;
}
