...
{
	global $strurl,$login_form,$sess_id,$grp_name,$grp_desc,$active_status,$password,$name,$comname,$address,$contactnum,$eml,$login_type,$user_type,$grp_id;
	...
	// --------------------------------------------------- LOGIN UPDATE QUERY--------------------------------------------------------
		if($login_type==3){
			...
			$sqlquery = "update access_detail set login='" . $grp_desc. "', password='" . $password . "', name='" . $name. "', address='" . $address. "', companyName='" . $comname. "', contactNo='" . $contactnum. "', email='" . $eml. "',user_type='" . $userType. "' where id='" .$grp_id. "' ";
		}
		else{
			// echo "</br>".$login_form." | ".$action." | ".$smenu." | ".$login_type." | ".$grp_desc." | ".$password." | ".$name." | ".$comname." | ".$address." | ".$contactnum." | ".$eml." | ".$grp_id." | ".$userType;
			//die();
