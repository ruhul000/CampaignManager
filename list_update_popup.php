...
$url="login=" . $login_form . "&sess_id=" . $sess_id ."&smenu=" . $smenu ."&schlr_name=" . $schlr_name ."&target_id=" . $target_id ."&s_date=" . $s_date ."&e_date=" . $e_date ."&s_hour=" . $s_hour ."&s_minute=" . $s_minute ."&e_hour=" . $e_hour ."&e_minute=" . $e_minute ."&rule_id=" . $rule_id ."&msgln=" . $msgln ."&list_id=" . $list_id ."&sms_type=" . $sms_type;
...
if ($action=="3" && $list_id){
	...
	header("Location: list_view_popup.php?login=" . $login_form . "&sess_id=" . $sess_id ."&list_id=" . $next_id . "&msg_alert=" . $msg_alert . "&smenu=" . $smenu);
	die();
}

if($action==1){
	$urlstr="Location: list_create_popup.php?" . $url;
}else if($action==2){
	$urlstr="Location: list_modify_popup.php?" . $url;
}
...
if ($action==1) {
	...
	header("Location: list_view_popup.php?login=" . $login_form . "&sess_id=" . $sess_id ."&list_id=" . $list_id . "&msg_alert=" . $msg_alert . "&smenu=" . $smenu);
	die();
}else if ($action==2) {
	getBaseDetail($target_id);
	getSmsDetail($rule_id);
	update_list_detail ($list_id);
	$url="login=" . $login_form . "&sess_id=" . $sess_id ."&smenu=" . $smenu ."&schlr_name=" . $schlr_name ."&target_id=" . $target_id ."&s_date=" . $s_date ."&e_date=" . $e_date ."&s_hour=" . $s_hour ."&s_minute=" . $s_minute ."&e_hour=" . $e_hour ."&e_minute=" . $e_minute ."&rule_id=" . $rule_id ."&msgln=" . $msgln ."&list_id=" . $list_id;
	//echo $urlstr;
...
{
	global $sender_id,$message,$footer_url,$sms_mode,$target_path,$dnd_id,$dnd_path,$sms_type,$login_form, $schlr_name, $target_id, $start_date, $end_date, $rule_id, $msgln, $s_hour, $s_minute, $e_hour, $e_minute, $daily_new_target ;
...
{
        global $sender_id ,$sms_type,$login_form, $schlr_name, $target_id, $start_date, $end_date, $rule_id, $msgln,$active_status,$daily_new_target;
